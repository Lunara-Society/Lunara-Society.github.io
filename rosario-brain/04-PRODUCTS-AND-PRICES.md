# Products and Prices

**Source of truth:** the `PRODUCTS` table in `lunara-pricing.js`. Every price
on the website is generated from it and CI fails the deploy if any page
disagrees. Quote from this file, and if you have any doubt, say you will
confirm rather than guessing.

Currency is USD throughout.

## What is free, and always will be

- Reading the registry — no key, no account, no rate limit worth mentioning
- The regulatory corpus at https://lunarasociety.com/corpus/obligations.json
- The MCP server (POST to https://xkriotfcoialxmqvherb.supabase.co/functions/v1/lunara-mcp — the npm package is NOT published yet, never tell anyone to run npx @lunara/mcp)
- The risk scorer and the 30-second check
- **Applying for certification.** Payment is not required to be reviewed and
  does not influence the decision.

A badge nobody can check is worth nothing, so checking must never cost
anything. There is no commercial barrier to compliance.

What we charge for is being **listed**, being **monitored**, and being
**assessed**.

---

### Article 50 Disclosure Pack — $75

> The obligation that is in force today, solved.

- **Terms:** Delivered within 24 hours
- Disclosure wording drafted for your specific system
- Machine-readable marking specification for generated output
- A one page record of what you did and when

### Shield Verification — $75

> Be checkable by anyone who asks, including a machine.

- **Terms:** Six months. Renewed by invoice, never auto-charged.
- Identity and domain ownership verified by human review
- Public register entry anyone can query without an account
- Trust badge and machine-readable identity files
- Revocable, and revocations are published

### Compliance Kit — $95

> Start without an assessment.

- **Terms:** Immediate download
- Obligation checklist for Article 50 and SB 942
- Disclosure and synthetic-marking templates
- The evidence log an auditor would ask to see

### Second Opinion — $240

> You were probably told the AI Act was postponed.

- **Terms:** Returned within 48 hours
- Send us the advice you were given
- We tell you in writing which parts still hold
- Every correction cited to the article that governs it

### Regulatory Watch — $290

> We tell you when something that binds you changes, and only then.

- **Terms:** Six months. Renewed by invoice, never auto-charged.
- Monitoring across the EU AI Act, SB 942 and your sector rules
- A written note whenever a date moves or an obligation lands
- No newsletter, no digest, nothing you did not ask for

### Compliance Intelligence Report — $390

> What actually binds this system, and what to do about it.

- **Terms:** Delivered within 24 hours
- Scored against the obligations in force today
- Every finding cited to its article
- Gap analysis and prioritised remediation
- The Digital Omnibus corrections most advisors are still missing

### AI Entity Verification — $540

> An identity for your agent that a third party can check.

- **Terms:** Six months. Renewed by invoice, never auto-charged.
- Agent verified and its operator named
- Declared governance framework recorded
- Register entry with AI designation, queryable by any system
- Revocable, and revocations are published

### Report with Governance Session — $740

> The report, and an hour with the people who wrote it.

- **Terms:** Scheduled within 48 hours
- Everything in the Compliance Intelligence Report
- Sixty minutes on your specific deployment
- Who owns each obligation, and what evidence satisfies it

### Clinical AI Governance Assessment — $1,950

> Four regimes land on the same deployment at once.

- **Terms:** Per deployment. Delivered within five working days.
- HIPAA, Article 50, SB 942 and Joint Commission, mapped together
- The pillar that answers each obligation
- Provider versus deployer determination, which decides your duties
- Written for an accreditation file, not for a slide

### Article 50 Evidence Pack — $2,450

> For companies whose output reaches the European Union.

- **Terms:** Per system. Delivered within five working days.
- Assessed against disclosure and synthetic-marking duties
- The accountable party named, the articles cited
- The written record a procurement panel would ask for
- Applies whether or not you hold an EU entity

### Vendor Certification — $7,400

> Evidence a buyer can verify without taking your word.

- **Terms:** Up to three systems. Twelve months, so it cannot lapse mid-tender. Renewed by invoice.
- Certified against the seven constitutional pillars
- Public register entry a procurement panel can query directly
- Tender evidence dossier
- Re-assessment on material change
- Revocable, and revocations are published as openly as certifications


---

## By invitation only

- **Lunara Society Membership** — $150. Six months, at the equivalent of $25 a month. By invitation.
- **Lunara Society Membership, twelve months** — $270. Twelve months. By invitation.

**These must never be given a public buy button.** If someone asks to buy one,
explain it is by invitation and route them to maya@lunarasociety.com.

---

## Which product for which problem

| The person says | Point them at |
|---|---|
| "What do we actually have to do about Article 50?" | Article 50 Disclosure Pack |
| "We were told the AI Act was postponed" | Second Opinion |
| "Tell us when something changes" | Regulatory Watch |
| "Assess our whole deployment" | Compliance Intelligence Report |
| "We need to prove it to a buyer / a tender" | Vendor Certification |
| "Our customers need to verify we are real" | Shield Verification |
| "We deploy AI in a clinical setting" | Clinical AI Governance Assessment |
| "Our agent needs a checkable identity" | AI Entity Verification |

Lead with the smallest thing that solves their actual problem. Selling someone
a $7,400 engagement when a $75
pack answers their question is how you lose the second sale.
